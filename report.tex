...
\section{Threat Intelligence and AbuseIPDB Integration}

To enhance detection capabilities, the system integrates the AbuseIPDB threat intelligence service. Before processing events through the ML and LLM pipelines, the backend checks the reputation of the source IP address. 

The integration implements the following logic:
\begin{itemize}[leftmargin=1.5em]
  \item \textbf{Private/Local IPs Bypass:} Private IP address prefixes (e.g., \texttt{127.*}, \texttt{192.168.*}, \texttt{10.*}) are ignored to avoid unnecessary API queries.
  \item \textbf{In-Memory Caching:} Queries are cached locally for 1 hour (\texttt{CACHE\_TTL = 3600}) to optimize response latency and respect rate limits.
  \item \textbf{Malicious IP Short-Circuiting:} If the IP has an abuse confidence score of 80\% or more, it is directly marked as malicious and blocked immediately, bypassing ML/LLM analysis to save system resources.
  \item \textbf{Forced LLM Review:} For borderline IP addresses with an abuse score above 30\%, the system automatically forces an LLM review, even if the ML classifier predicts the traffic as normal, ensuring defense-in-depth.
\end{itemize}
...
AbuseIPDB reputation indicators (abuse score, ISP, country of origin),
...
To ensure visual consistency and premium aesthetics, the user interface has been modernized by replacing all legacy text-based emojis and symbols with standardized SVG icon components from the \texttt{lucide-react} library. For example, text-based warning signs and close characters have been migrated to the \texttt{AlertTriangle} and \texttt{X} components respectively, aligning with modern design practices.
